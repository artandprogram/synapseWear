...
- (void)setUuids
{
    ...
    // For Test
    //services = nil;
    /*NSMutableArray *servicesTest = [NSMutableArray array];
    for (int i=0; i<pow(16, 4); i++) {
        CBUUID *uuid = [CBUUID UUIDWithString:[NSString stringWithFormat:@"%04X", i]];
        NSLog(@"uuid: %@", uuid);
        [servicesTest addObject:uuid];
    }
    services = [servicesTest mutableCopy];*/
    //NSLog(@"setUuids services: %@", services);
}
...
- (bool)isBluetoothLESupported
{
    ...
    NSLog(@"isBluetoothLESupported: %@", message);

    if ([delegate respondsToSelector:@selector(shouldDisplayAlertTitled:messageBody:peripheral:)]) {
        [delegate shouldDisplayAlertTitled:@"Bluetooth LE Support"
                               messageBody:message
                                peripheral:nil];
    }
...
- (void)connectRFduino:(RFduino *)rfduino
{
    ...
    [central connectPeripheral:[rfduino peripheral]
                       options:nil];
}

- (void)disconnectRFduino:(RFduino *)rfduino
{
    NSLog(@"disconnectRFduino");
  ...
  didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    if (rfduino) {
        [rfduino setCustomUUID:customUUID];
        [rfduino connected];
...
didDisconnectPeripheral:(CBPeripheral *)peripheral
                 error:(NSError *)error
{
    NSLog(@"RFduinoManager didDisconnectPeripheral");
    ...
    if (error.code) {
        ...
        if ([delegate respondsToSelector:@selector(shouldDisplayAlertTitled:messageBody:peripheral:)]) {
            [delegate shouldDisplayAlertTitled:@"Peripheral Disconnected with Error"
                  ...
                  RSSI:(NSNumber *)RSSI
{
    ...
    if (manufacturerData) {
        ...
        NSData *data = [NSData dataWithBytes:bytes+2
                                      length:len-2];
                 ...
                 error:(NSError *)error
{
    ...
    if ([delegate respondsToSelector:@selector(shouldDisplayAlertTitled:messageBody:peripheral:)]) {
        [delegate shouldDisplayAlertTitled:@"Connect Failed"
                               messageBody:error.description
                                peripheral:peripheral];
    }
}
