...
class AccessKeysFileManager: BaseFileManager {
    ...
    let accessKeyDataKey: String = "access_key"
        ...
        if let accessKeys = self.getAccessKeysData() {
            for accessKey in accessKeys {
                if let accessKey = accessKey as? [String: Any], let uuidVal = accessKey[self.uuidKey] as? UUID, uuidVal == uuid {
                    if let accessKeyData = accessKey[self.accessKeyDataKey] as? Data {
                        res = accessKeyData
                    }
                    break
                }
    ...
    func setAccessKey(_ uuid: UUID, accessKey: Data) -> Bool {
        ...
        if pt < accessKeys.count {
            accessKeys[pt] = [self.uuidKey: uuid, self.accessKeyDataKey: accessKey, self.dateKey: Date()]
        }
        else {
            accessKeys.append([self.uuidKey: uuid, self.accessKeyDataKey: accessKey, self.dateKey: Date()])
        }
