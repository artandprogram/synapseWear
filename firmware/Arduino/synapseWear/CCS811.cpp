 ...
 * Please see https://github.com/adafruit/Adafruit_CCS811
 * 
 * Forked and modified for synapseWear
 */
#include "CCS811.h"

CCS811::CCS811(uint8_t PIN_WAKEAIR)
{
  _PIN_WAKEAIR = PIN_WAKEAIR;
  pinMode(_PIN_WAKEAIR, OUTPUT);
  digitalWrite(_PIN_WAKEAIR, HIGH);
...
bool CCS811::begin(uint8_t addr)
{
  ...
  // needs delay before being ready
  delay(100);
  ...
  // calculate temperature offset until ccs811 is ready
}
...
bool CCS811::lowPower()
{
  setDriveMode(CCS811_DRIVE_MODE_60SEC);
  isEnabled = true;
}

bool CCS811::enable(uint8_t mode)
{
  if (isEnabled)
    return true;
...
{
  _eCO2 = _TVOC = 65535;
  setDriveMode(CCS811_DRIVE_MODE_IDLE);
  isEnabled = false;
}
